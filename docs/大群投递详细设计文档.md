...
> **在线判定原则（重要 / 当前实现）**
>
> 「在线」是**用户登录态的全局维度**：用户建立 WebSocket 连接（登录）即视为在线，
> 与是否进入某个群无关。投递时只需判断「群成员里谁当前登录在线」，
> **不为每个群维护独立的在线集合**。
>
> 因此下文方案二 `group:{groupId}:online_users`、方案三按 serverId 维护群在线集合，
> 都是**群维度**设计，与本系统「登录即在线」的理念冲突，**当前不采用、未实现**，
> 仅作历史方案保留。当前采用方案一（枚举群成员 + 用户登录态过滤）。
...
```text
消息校验 + 落库（group_message）
  ↓ 同事务写 message_outbox
返回 ACK 给发送者
  ↓
Outbox Relay（每 500ms 轮询）可靠投递到 Kafka（key=groupId）
  ↓
Delivery 消费（processMessage：失败短重试 3 次，成功才提交 offset）
  ↓
handle 按 eventType 分派：
  - 群消息/撤回 → fanout（全量活跃成员 + 在线过滤）
  - 结构化事件 → handleStructuredEvent（按 targetUserIds）
  ↓
resolveOnlineRoutes：user → connections → server 分组（map[serverId][]connectionId）
  ↓
按 serverId 调各节点 POST /internal/push（携带 connectionIds）
  ↓
WS 节点 SendToConnections 推送到本机对应连接
```

### 27.2 在线成员筛选：采用方案一（已实现）

- 在线判定为**用户登录态全局维度**（建立 WS 连接即在线，与进群无关），
  因此**有意不采用群维度在线集合**。
- 当前走 **§8.1 方案一**：`fanout` 先 `ListActiveMemberIDs` 分页枚举全量活跃成员
  （每页 1000），再 `resolveOnlineRoutes` 按用户登录态（`online:user:{uid}:connections`）
  解析在线连接。
- **§8.2 方案二** `group:{groupId}:online_users` 在线集合：**不采用**（群维度，与设计理念冲突）。
- **§8.3 方案三** 按 serverId 维护群在线集合：**不采用**（同上）。
- 若未来要降低大群成员枚举压力，推荐方向是「全局在线用户集合 ∩ 群成员」，
  而非 per-group 集合。

### 27.3 连接路由：连接级（多端 / 多节点已支持）

- `resolveOnlineRoutes` 读取 `online:user:{uid}:connections`（集合）→
  对每个连接读 `connection:{cid}:server`，按 serverId 分组，返回
  `serverId -> []connectionId`，无 `connection:{cid}:server` 的脏连接被跳过并计数。
- 节点推送地址来自 `server:{serverId}:push_url`；缺省回落 `cfg.InternalPushURL`。

### 27.4 内部推送接口（当前实现）

- 路径：**`POST /internal/push`**（非文档示例 `/internal/ws/push`）。
- 请求体：`{ "connectionIds": [...], "type": wsType, "data": <payload> }`
  （Delivery 始终发 connectionIds，不发 targetUserIds）。
- 响应体：`{ "data": { "pushed": N } }`。
- 批大小：结构化事件与群消息推送均按 **500** 分批；成员分页 1000。

### 27.5 失败与重试（当前实现）

- Delivery 消费失败：内存短重试 `{0, 50ms, 100ms}` 共 3 次，成功才提交 offset；
  仍失败则不提交、记日志，靠 Kafka 重投与客户端补拉兜底。
- 顺带修复：`ListActiveMemberIDs` 分页游标边界漏成员、结构化事件 fanout 缺 500 分批。
- 无 DLQ topic。

### 27.6 节点存活与脏数据清理（已实现）

- WS 节点每 10s 写 `server:{serverId}:heartbeat`（TTL 30s）。
- Delivery `RunCleanup`（每 30s）`SCAN server:*:connections` 枚举节点，
  heartbeat 过期即判死节点，清理其遗留的 `connection:{cid}:server`、
  `connection:{cid}:user`，并从 `online:user:{uid}:connections` 移除脏连接。

### 27.7 限流（已实现）

- 慢速模式：`rate_limit:group:{groupId}:user:{userId}`（SetNX，TTL=slowModeSeconds）。
- 全局发送：`rate_limit:user:{userId}:send_message`（1 秒窗口、阈值 5）。
- @所有人：群维度 `rate_limit:group:{groupId}:mention_all` + 用户维度
  `rate_limit:user:{userId}:mention_all`（SetNX，普通 60s / 大群 300s）。
- 均在 Message Service 发送前校验，Delivery 不做限流。

### 27.8 未实现 / 待演进

- 热点群标记与自动降级（`hot_group:{groupId}` / `:strategy`）：未实现。
- 在线优先投递（`online_users` 集合 / 单用户 TTL key）：未实现（已回退）。
- 按 serverId 维护群在线集合、超级热点群独立 topic、动态 batch、消息合并推送：未实现。
- Producer 仍为 `RequireOne`（可后续改 `acks=all`）。
